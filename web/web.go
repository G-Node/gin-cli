...
func (cl *Client) Get(address string) (*http.Response, error) {
	...
	if err != nil {
		return nil, Error{UError: err.Error(), Origin: fmt.Sprintf("Get(%s)", requrl)}
	}
...
func (cl *Client) Post(address string, data interface{}) (*http.Response, error) {
	...
	if err != nil {
		return nil, Error{UError: err.Error(), Origin: fn}
	}
	requrl := urlJoin(cl.Host, address)
	...
	resp, err := cl.web.Do(req)
	if err != nil {
		err = Error{UError: err.Error(), Origin: fn}
	}
	return resp, err
}
...
func (ut *UserToken) LoadToken() error {
	...
	if err != nil {
		return Error{UError: err.Error(), Origin: fn, Description: "failed to load user token"}
	}
	...
	if err != nil {
		return Error{UError: err.Error(), Origin: fn, Description: "failed to parse user token"}
	}
...
func (ut *UserToken) StoreToken() error {
	...
	if err != nil {
		return Error{UError: err.Error(), Origin: fn}
	}
	filepath := filepath.Join(path, "token")
	file, err := os.Create(filepath)
	if err != nil {
		util.LogWrite("Failed to create token file %s", filepath)
		return Error{UError: err.Error(), Origin: fn, Description: fmt.Sprintf("failed to create token file %s", filepath)}
	}
	defer closeFile(file)
	...
	if err != nil {
		...
		return Error{UError: err.Error(), Origin: fn, Description: "failed to store token"}
	}
	util.LogWrite("Saved")
	return nil
}
...
func DeleteToken() error {
	...
	if err != nil {
		return Error{UError: err.Error(), Origin: "DeleteToken()", Description: "could not delete token"}
	}
